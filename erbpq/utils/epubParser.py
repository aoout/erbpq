...
class EpubParser:
    def __init__(self, epubPath: str) -> None:
        ...
        self.pagesPath, self.css_path = self.__parse()
        self.toc = self.__parseToc()
        self.meta = self.__parseMeta()
    ...
    def __parse(self) -> Tuple[List[Path], List[Path]]:
    ...
    def getPageHtml(self, pageIndex: int, withCss: bool = True) -> str:
        with open(self.pagesPath[pageIndex], "r", encoding='utf-8') as f:
            html = f.read()
        if withCss:
